...
while myStart < myMaxStart:
    ...
    try:
        urls = googlesearch.search(query=myKeyword, tld=myTLD, lang=myHl, safe='off',
                                   num=myNum, start=myStart, stop=myStop, pause=myPause,
        ...
        # df['tldLang'] = myTLDLang  #fill with current country / tld lang not use here all in english
...
dfPagesAnswers = pd.DataFrame(
    columns=['keyword', 'page', 'position', 'BERT_score', 'source', 'search_date', 'answers', 'starts', 'ends',
             'local_probs', 'total_probs'])

for i in range(0, len(dfPagesUnique)):
    soup = ""
    print(
        "Page keyword tldLang  i = " + dfPagesUnique.loc[i, 'page'] + " " + dfPagesUnique.loc[i, 'keyword'] + " " + str(
            i))
